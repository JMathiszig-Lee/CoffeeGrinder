...
class Seesaw:
    ...
    def __init__(self, i2c_bus, addr=0x49, drdy=None, reset=True):
        ...
        self.pin_mapping = None  # Skipped pin mapping setup for MicroPython

    def sw_reset(self, post_reset_delay=1.0):
        try:
    ...
    def read(self, reg_base, reg, buf, delay=0.008):
        self.i2c.writeto(self.addr, bytes([reg_base, reg]))
        time.sleep(delay)
        self.i2c.readfrom_into(self.addr, buf)
    

    def write(self, reg_base, reg, buf=None):
        data = bytearray([reg_base, reg])
        if buf:
            data += buf
